...
def main():
    ...
    level_counts = Counter((e["verification"]["level"] or "unknown").split()[0]
                           for e in specials["entries"])
    level_summary = ", ".join(f"{k} {v}" for k, v in sorted(level_counts.items()))
    print(f"lunch entries: {len(specials['entries'])} ({level_summary})")
    ...
    if not as_text:
        ...
        with open(os.path.join(DATA, "generated.js"), "w", encoding="utf-8") as fh:
            fh.write("/* Generated by scripts/build_data.py - do not edit; edit data/*.json instead.\n"
                     "   Trip: Tue 2026-09-08. Verified against SFMTA, Caltrain, VTA, and per-row restaurant/listing/review sources.\n"
